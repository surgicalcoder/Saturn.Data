...
public abstract class ComprehensiveScopedRepositoryContractTests<TFixture, TRepository>(TFixture fixture) : IAsyncLifetime
    where TFixture : IRepositoryTestFixture<TRepository>
    where TRepository : IRepository, IScopedRepository, IScopedReadonlyRepository
{
    ...
    {
        // Arrange
        await Repo.Insert(new List<ParentScope> { WellKnownData.ParentScope1, WellKnownData.ParentScope2 });
        ...
        // Act
        await ScopedRepo.Insert<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity1);
        await ScopedRepo.Insert<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity2);

        // Assert
        var scope1Count = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, e => true);
        var scope2Count = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, e => true);
        
        Assert.Equal(1, scope1Count);
        Assert.Equal(1, scope2Count);
        
        var scope1Entity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity1.Id);
        var scope2Entity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity2.Id);
        ...
        var scope1EntityInScope2 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity1.Id);
        var scope2EntityInScope1 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity2.Id);
    ...
    public async Task Insert_Multiple_Entities_Should_Not_Leak_Between_Scopes()
    {
        ...
        // Act
        await ScopedRepo.Insert<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, scope1Entities);
        await ScopedRepo.Insert<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, scope2Entities);
        ...
        var scope1All = await (await ScopedReadonlyRepo.All<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id)).ToListAsync();
        var scope2All = await (await ScopedReadonlyRepo.All<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id)).ToListAsync();
        
        Assert.Equal(2, scope1All.Count);
        Assert.Equal(2, scope2All.Count);
        
        Assert.All(scope1All, e => Assert.Equal(WellKnownData.ParentScope1.Id, e.Scope));
        Assert.All(scope2All, e => Assert.Equal(WellKnownData.ParentScope2.Id, e.Scope));
    }
    ...
    public async Task Update_Should_Only_Affect_Entities_In_Same_Scope()
    {
        ...
        await ScopedRepo.Update<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity1);

        // Assert
        var updatedEntity1 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity1.Id);
        var unchangedEntity2 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity2.Id);
    ...
    public async Task Save_Should_Respect_Scope_Boundaries()
    {
        ...
        await ScopedRepo.Save<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity);
        
        // Verify entity exists in scope 1
        var scope1Entity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity.Id);
        Assert.NotNull(scope1Entity);
        Assert.Equal("Save Test Entity", scope1Entity.Name);
        Assert.Equal(WellKnownData.ParentScope1.Id, scope1Entity.Scope);
        ...
        await ScopedRepo.Save<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity);

        // Assert - The entity should now be in scope 2 only (moved, not duplicated)
        var scope1EntityAfterMove = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity.Id);
        var scope2Entity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity.Id);
        ...
        Assert.Equal(WellKnownData.ParentScope2.Id, scope2Entity.Scope);
    ...
    public async Task Delete_By_Id_Should_Only_Delete_From_Specified_Scope()
    {
        ...
        await ScopedRepo.Delete<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity1.Id);
        ...
        await ScopedRepo.Delete<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity1.Id);
        
        // Entity in scope 2 should still exist since the delete was scoped
        var scope2EntityAfterWrongScopeDelete = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity2.Id);
    ...
    public async Task Delete_By_Filter_Should_Only_Delete_From_Specified_Scope()
    {
        ...
        await ScopedRepo.Delete<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, e => e.Name == "Filter Delete Test");
        ...
        var scope1Remaining = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, scope1Entities[1].Id);
        var scope2FilterEntity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, scope2Entities[0].Id);
    ...
    public async Task Delete_Multiple_IDs_Should_Only_Delete_From_Specified_Scope()
    {
        ...
        await ScopedRepo.Delete<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, new[] { scope1Entities[0].Id, scope1Entities[1].Id });
        ...
        var scope2Entity1 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, scope2Entities[0].Id);
        var scope2Entity2 = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, scope2Entities[1].Id);
        ...
        var scope1CorrectRemaining = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, scope1Entities[2].Id);
    ...
    public async Task Upsert_Should_Respect_Scope_Boundaries()
    {
        ...
        // Insert into scope 1
        await ScopedRepo.Insert<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity);

        // Verify entity exists in scope 1
        var scope1EntityAfterInsert = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity.Id);
        Assert.NotNull(scope1EntityAfterInsert);
        Assert.Equal("Original Upsert Entity", scope1EntityAfterInsert.Name);
        Assert.Equal(WellKnownData.ParentScope1.Id, scope1EntityAfterInsert.Scope);
        ...
        await ScopedRepo.Upsert<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity);

        // Assert - The entity should now be in scope 2 only (moved, not duplicated)
        var scope1EntityAfterUpsert = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, entity.Id);
        var scope2Entity = await ScopedReadonlyRepo.ById<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity.Id);
    ...
    public async Task Count_Should_Only_Count_Entities_In_Specified_Scope()
    {
        ...
        // Act & Assert
        var scope1TotalCount = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, e => true);
        var scope2TotalCount = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, e => true);
        var scope1FilteredCount = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope1.Id, e => e.Name == "Count Test");
        var scope2FilteredCount = await ScopedReadonlyRepo.Count<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, e => e.Name == "Count Test");
    ...
    public async Task Many_Should_Only_Return_Entities_From_Specified_Scope()
    {
        ...
        Assert.All(scope1Results, e => Assert.Equal(WellKnownData.ParentScope1.Id, e.Scope));
        Assert.All(scope2Results, e => Assert.Equal(WellKnownData.ParentScope2.Id, e.Scope));
    }
    ...
    public async Task One_Should_Only_Return_Entity_From_Specified_Scope()
    {
        ...
        Assert.Equal(WellKnownData.ParentScope1.Id, scope1Result.Scope);
        Assert.Equal(WellKnownData.ParentScope2.Id, scope2Result.Scope);
    }
    ...
    public async Task Random_Should_Only_Return_Entities_From_Specified_Scope()
    {
        ...
        Assert.All(scope1Random, e => Assert.Equal(WellKnownData.ParentScope1.Id, e.Scope));
        Assert.All(scope2Random, e => Assert.Equal(WellKnownData.ParentScope2.Id, e.Scope));
    }
    ...
    public async Task Complex_Multi_Scope_Operations_Should_Maintain_Data_Integrity()
    {
        ...
        await Repo.Insert(new List<ParentScope> { WellKnownData.ParentScope1, WellKnownData.ParentScope2, scope3 });
        ...
        await ScopedRepo.Update<ChildEntity, ParentScope>(WellKnownData.ParentScope2.Id, entity2);
